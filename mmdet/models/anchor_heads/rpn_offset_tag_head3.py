...
from ..losses import TagOffsetLoss
...
class RPN_Offset_TAGHead3(AnchorHead):
    ...
    def __init__(self, in_channels, tag_anchor_free = True, **kwargs):
        ...
        super(RPN_Offset_TAGHead3, self).__init__(2, in_channels, **kwargs)
    ...
    def loss(self,
        ...
        for img_idx in range(img_num):
            ...
            imgs_tag.append(tag_pred[0][img_idx]) 
            imgs_offset.append(bbox_preds[0][img_idx].permute(1,2,0).contiguous().view(-1, 4))
        
        # assert isinstance(self.loss_tag, TagOffsetLoss), 'ONLY support TagOffsetLoss!'
        def get_trans_center(anchors, offsets):
            proposals = delta2bbox(anchors, offsets, self.target_means,
                                   self.target_stds, img_metas[0]['pad_shape'][:2])
        ...
        losses_tag = self.loss_tag(imgs_tag, all_bbox_gt_inds, all_bbox_anchor_inds,
                    anchor_list[0], imgs_offset, get_trans_center, gt_bboxes)
