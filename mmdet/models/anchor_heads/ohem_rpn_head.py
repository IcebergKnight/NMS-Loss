...
class OHEMRPNHead(AnchorHead):

    def __init__(self, in_channels, **kwargs):
        super(OHEMRPNHead, self).__init__(2, in_channels, **kwargs)
    ...
    def get_bboxes_single(self,
        ...
        for idx in range(len(cls_scores)):
            ...
            if self.use_sigmoid_cls:
                ...
                rpn_cls_score = torch.unsqueeze(scores,1)
            else:
            ...
            # proposals = torch.cat([proposals, scores.unsqueeze(-1), rpn_cls_score], dim=-1)
            if proposals.dim() == 1:
                continue
