...
class TwoStageDetector(BaseDetector, RPNTestMixin, BBoxTestMixin,
    ...
    def extract_feat(self, img):
        ...
        if self.with_neck: 
            x = self.neck(x)
            # self.visible_heatmap(img, self.neck.atten_heatmaps, 'pred')
        return x
    
    # just used for debug
    def visible_heatmap(self, img_tensor, heatmap_tensors, post_fix = 'pred'):
        alpha = 0.5
        dst_dir = '/home/joeyfang/fzcode/research/mmdetection/visible_heatmap_' + post_fix
        current_milli_time = lambda: int(round(time.time() * 1000))
        assert img_tensor.size()[0] == heatmap_tensors[0].size(0)
        mean = np.array([123.675, 116.28, 103.53])
        std = np.array([58.395, 57.12, 57.375])
        # 1.get single img to numpy
        # 2.trans [c,h,w] to [h,w,c]
        # 3.trans rgb to bgr for using cv2
        # 4.get single heatmap to numpy
        # 5.expand heatmap to img size
        # 6.merge img and heatmap
        # 7.save
        for i in range(img_tensor.size()[0]):
            ...
            for scale_idx, heatmap_tensor in enumerate(heatmap_tensors):
                heatmap = heatmap_tensor[i].detach().cpu().numpy()
                heatmap = heatmap.transpose(1,2,0).squeeze()
                heatmap = cv2.resize(heatmap, img.shape[:2][::-1])
                cmap = plt.get_cmap('jet')
                rgba_img = cmap(heatmap)
                rgb_img = np.delete(rgba_img, 3, 2)
                heatmap = ((rgb_img * 254)[:,:,::-1]).astype(np.int32)
                s_img = img.copy().astype(np.int32)
                cv2.addWeighted(heatmap, alpha, s_img, 1-alpha, 0, s_img)
    ...
    def forward_train(self,
        ...
        # AttentionFPN loss
        if hasattr(self.neck, 'act_feat_num'):
            neck_losses = self.neck.loss(gt_bboxes, img_meta, gt_bboxes_ignore)
            losses.update(neck_losses)
            # self.visible_heatmap(img, self.neck.gt_hmp_tensor, 'gt')
        # RPN forward and loss
        if self.with_rpn:
            rpn_outs = self.rpn_head(x)
            if self.rpn_head.__class__.__name__ == "RPN_NMS_Head":
        ...
        if self.with_bbox:
            ...
            cls_score, bbox_pred, tag_pred = self.bbox_head(bbox_feats)
            bbox_targets, pos_assigned_gt_inds = self.bbox_head.get_target(
                sampling_results, gt_bboxes, gt_labels, self.train_cfg.rcnn)
            loss_bbox = self.bbox_head.loss(cls_score, bbox_pred, tag_pred, 
                                             pos_assigned_gt_inds, *bbox_targets)
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        det_bboxes, det_labels = self.simple_test_bboxes(
            x, img_meta, proposal_list, self.test_cfg.rcnn, rescale=rescale, img = img)
