...
class RPN_Feat_RGBRCNN(BaseDetector, RPNTestMixin, BBoxTestMixin,
                       MaskTestMixin):
    ...
    def __init__(self,
                 ...
                 test_cfg=None,
                 pretrained=None,
                 featnet_pretrained=None,
                 rcnn_pretrained=None,
                 fix_rpn = False,
                 fix_rcnn = False,
                 generate_xgb_data = False,
                 feat_img_pool_stride = None):
        super(RPN_Feat_RGBRCNN, self).__init__()
        assert featnet_backbone is not None, 'Use RPN_RGBRCNN instead'
        self.rpn_part = []
        self.rcnn_part = []
        self.fix_rpn = fix_rpn
        self.fix_rcnn = fix_rcnn
        self.generate_xgb_data = generate_xgb_data
        self.feat_img_pool_stride = feat_img_pool_stride
        if feat_img_pool_stride is not None:
            feat_roi_extractor['featmap_strides'] *= feat_img_pool_stride
        print('merge feature stride:', feat_roi_extractor['featmap_strides'])
        ...
        if featnet_backbone is not None:
            self.featnet_backbone = builder.build_backbone(featnet_backbone)
            self.rcnn_part.append(self.featnet_backbone)
        ...
        self.init_weights(pretrained=pretrained, 
                          featnet_pretrained = featnet_pretrained,
                          rcnn_pretrained = rcnn_pretrained)
        self._freeze()
    ...
    @property
    def with_featnet_backbone(self):
        return hasattr(self, 'featnet_backbone') and self.featnet_backbone is not None

    def init_weights(self, pretrained=None, 
                           featnet_pretrained=None,
                           rcnn_pretrained=None):
        super(RPN_Feat_RGBRCNN, self).init_weights(pretrained)
        ...
        if self.with_featnet_backbone:
            self.featnet_backbone.init_weights(featnet_pretrained)
    ...
    def forward_train(self,
        ...
        if self.with_bbox and not self.fix_rcnn:
            ...
            # parse whole img to extract feature
            feat_img = img
            if self.feat_img_pool_stride is not None:
                t_img_shape = [int(img.size()[-2]/ self.feat_img_pool_stride), 
                               int(img.size()[-1]/ self.feat_img_pool_stride)]
                feat_img = nn.AdaptiveAvgPool2d(t_img_shape)(img)
            featnet_feat = self.featnet_backbone(feat_img)
            featnet_feat = featnet_feat[0]
            # img_extrator to crop img and feature
            bbox_imgs, expand_rois = self.bbox_roi_extractor(img, rois)
            roi_feats, _ = self.feat_roi_extractor(featnet_feat, rois)
            rcnn_feat = roi_feats
            if self.with_rcnn_backbone:
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        # print('proposal_list')
        # print(proposal_list)
        featnet_feat = self.featnet_backbone(img)
        featnet_feat = featnet_feat[0]
        det_bboxes, det_labels, rcnn_score = self.rgb_simple_test_bboxes(
            ...
            extra_feats = featnet_feat, rescale=rescale)
