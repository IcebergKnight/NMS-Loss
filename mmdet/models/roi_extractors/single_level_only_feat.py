...
class Feat_ROI_Extractor(nn.Module):
    ...
    def __init__(self,
                 ...
                 expand_ratio,
                 featmap_strides):
        super(Feat_ROI_Extractor, self).__init__()
    ...
    def build_img_roi_layers(self, layer_cfg):
        ...
        roi_layer = layer_cls(spatial_scale=1 / self.featmap_strides, **cfg)
        return roi_layer

    def forward(self, feats, rois):
        # rois: Tensor: shape (n, 5), [batch_ind, x1, y1, x2, y2]
        ori_rois = copy.deepcopy(rois)
        img_height = feats.size(2)
        img_width = feats.size(3)
        ...
        return self.img_roi_layer(feats, ori_rois), ori_rois
